...
% Pocket branch
\node[box, below=1.25cm of molinput] (pocketlmdb) {Candidate-pocket\\library};
\node[box, right=of pocketlmdb] (pocketenc) {Pocket\\encoder};
\node[box, right=of pocketenc] (pocketemb) {Cached pocket\\embeddings};

% Shared scoring and output
\node[box, right=1.35cm of pocketemb, yshift=0.62cm] (sim) {Similarity\\computation};
\node[box, right=of sim] (ranked) {Ranked list of\\candidate pockets};

% Arrows
\draw[arrow] (molinput) -- (molenc);
\draw[arrow] (molenc) -- (molemb);

\draw[arrow] (pocketlmdb) -- (pocketenc);
